...
namespace Netsphere.Game.GameRules
{
    internal sealed class TouchdownTrainingGameRule : GameRuleBase
    {
        ...
        private readonly TouchdownTrainingAssistHelper _TouchdownTrainingAssistHelper = new TouchdownTrainingAssistHelper();
        ...
        public void OnScoreOffense(Player killer, Player assist, Player target, AttackAttribute attackAttribute, LongPeerId Target, LongPeerId Killer, LongPeerId Assist)
        {
            if (IsInTouchdownTraining)
                return;
            ...
            if (assist != null)
                Room.Broadcast(new ScoreOffenseAssistAckMessage(new ScoreAssistDto(Killer, Assist, Target, attackAttribute)));
            else
                Room.Broadcast(new ScoreOffenseAckMessage(new ScoreDto(Killer, Target, attackAttribute)));
        }

        public void OnScoreDefense(Player killer, Player assist, Player target, AttackAttribute attackAttribute, LongPeerId Target, LongPeerId Killer, LongPeerId Assist)
        {
            if (IsInTouchdownTraining)
                return;
            ...
            if (assist != null)
                Room.Broadcast(new ScoreDefenseAssistAckMessage(new ScoreAssistDto(Killer, Assist, Target, attackAttribute)));
            else
                Room.Broadcast(new ScoreDefenseAckMessage(new ScoreDto(Killer, Target, attackAttribute)));
        }

        public void OnScoreRebound(Player newPlr, Player oldPlr, LongPeerId NewTarget, LongPeerId OldTarget)
        {
            if (IsInTouchdownTraining)
                return;
                
            if(oldPlr != null)
                _TouchdownTrainingAssistHelper.Update(oldPlr);

            if (newPlr != null)
                GetRecord(newPlr).OffenseReboundScore++;

            Room.Broadcast(new ScoreReboundAckMessage(NewTarget, OldTarget));
        }
