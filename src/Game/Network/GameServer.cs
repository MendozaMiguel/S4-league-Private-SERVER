...
namespace Netsphere.Network
{
    internal class GameServer : ProudServer
    {
        ...
        public static void Initialize(Configuration config)
        {
            ...
            config.MessageFactories = new MessageFactory[] { new RelayMessageFactory(), new GameMessageFactory(), new GameRuleMessageFactory(), new ClubMessageFactory() };
            ...
            : base(config)
        {
            ...
            _worker = new ThreadLoop(TimeSpan.FromMilliseconds(1000), (Action<TimeSpan>)Worker);
        ...
        protected override void OnDisconnected(ProudSession session)
        {
            ...
            if (gameSession.Player != null)
            {
                ...
                Logger.ForAccount(gameSession)
                    .Debug($"Client {session.RemoteEndPoint.ToString()} disconnected");
        ...
        protected override void OnError(ErrorEventArgs e)
        {
            ...
            if (e.Exception.ToString().Contains("opcode") || e.Exception.ToString().Contains("Bad format in"))
            {
                ...
                if (gameSession != null)
                    gameSession.SendAsync(new ServerResultAckMessage(ServerResult.FailedToRequestTask));
            }
            else if (gameSession.Player != null && ((gameSession.Player.Room != null && gameSession.Player.Room.GameRuleManager.GameRule.StateMachine.State == GameRuleState.Waiting) 
